...
- [ ] **Follow-up:** slice/array ops still full reload — should patch panel in place like `saveMeta` (see §2.3)
...
- [ ] In-place panel updates for slice/array mutations (no full reload)
...
- [ ] Extract `CmsSidebarFieldList.vue` (props: nodes, fieldsById, fields, busy flags; emits: field-click, add-item, remove-item)
...
- [ ] Extract one helper, e.g. `useGuestCachedAsyncData(key, fetcher)`
...
- [ ] Move to `app/fields/resolveField.ts` (or similar)
...
- [ ] Shared internal helper `loadFieldsForOwner(...)`
...
- [ ] Import canonical helpers from package / shared test utils
...
- [ ] Unify on one mutation strategy (prefer in-place panel patch)
...
- [ ] Pass `PageContent` (or `{ arrayField, itemSchema }`) from `useCmsPageActions` instead
...
- [ ] Fix docs to match reality, or add the parameter intentionally
...
- [ ] Delete until global editing exists, or wire through modal path
...
4. [ ] **Extract `CmsSidebarFieldList`** — kill template duplication
5. [ ] **Extract `useGuestCachedAsyncData`** — single static-first cache contract
6. [ ] **Move `resolveField` / `resolveArrayItems`** out of `usePageContent.ts`; fix `docs/field-types.md`
7. [ ] **Deduplicate E2E seeding** against package `seedFieldsFromSchema`
8. [ ] **Delete dead `updateGlobalFieldValue`** or implement global editing
...
- [ ] No duplicated sidebar field-tree template
- [ ] No copy-pasted `getCachedData` guard (4×)
- [ ] No wrong-layer exports (`resolveField` in fetch module)
- [ ] No dead public API (`updateGlobalFieldValue`)
- [ ] No wrong public docs (`resolveField` signature)
