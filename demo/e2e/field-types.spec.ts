...
test('editor can edit link and richtext on demo CTA slice', async ({ page }) => {
  ...
  await expect(page.locator('.cta-slice a')).toHaveText(DEMO_BASELINE.ctaLinkLabel)

  await page.locator('.cta-slice a').click()
  const dialog = page.locator('dialog.field-edit-modal')
  await expect(dialog).toBeVisible()
  await dialog.getByLabel('Label').fill(editedLabel)
  await dialog.getByRole('button', { name: 'Save' }).click()
  await expect(dialog).not.toBeVisible()
  await expect(page.locator('.cta-slice a')).toHaveText(editedLabel)

  await page.locator('.cta-slice .cms-richtext').click()
  await expect(dialog).toBeVisible()
  await dialog.locator('textarea').fill(editedSource)
  await dialog.getByRole('button', { name: 'Save' }).click()
  await expect(dialog).not.toBeVisible()

  const richtext = page.locator('.cta-slice .cms-richtext')
  await expect(richtext).toContainText('Updated')
  await expect(richtext.locator('strong')).toHaveText('richtext')

  await page.reload()
  await expect(page.locator('.cta-slice a')).toHaveText(editedLabel)
  const richtextAfterReload = page.locator('.cta-slice .cms-richtext')
